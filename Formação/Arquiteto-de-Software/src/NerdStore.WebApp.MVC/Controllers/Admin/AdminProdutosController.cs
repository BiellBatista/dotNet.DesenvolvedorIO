...
internal sealed class AdminProdutosController : Controller
{
    ...
    [Route("novo-produto")]
    public async Task<IActionResult> NovoProduto() => View(await PopularCategorias(new ProdutoViewModel()));
    ...
    public async Task<IActionResult> AtualizarEstoque(Guid id) => View("Estoque", await _produtoAppService.ObterPorId(id));
